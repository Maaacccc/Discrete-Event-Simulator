package cs2030.simulator;

import cs2030.util.Pair;
import java.util.Optional;

class ArriveEvent implements Event {
    private final Customer customer;
    private final double time;
    private final int serverIndex;

    public ArriveEvent(Customer customer, double time) {
        this.customer = customer;
        this.time = time;
        this.serverIndex = -1;
    }

    @Override
    public String getType() {
        return "ARRIVE";
    }

    @Override
    public int getCustomerId() {
        return customer.getId();
    }

    @Override
    public boolean equals(Event other) {
        if (this.getType().equals(other.getType()) &&
                this.getCustomerId() == other.getCustomerId() &&
                this.getTime() == other.getTime()) {
            return true;
        } else {
            return false;
        }
    }

    @Override
    public double getTime() {
        return this.time;
    }

    @Override
    public Customer getCustomer() {
        return this.customer;
    }

    @Override
    public Pair<Optional<Event>, Shop> execute(Shop shop) {
        /**
         * Proceed from ARRIVE to the next status. Iterates through the serverList to check if
         * there is any server available to serve the customer immediately. If there isn't, it will
         * loop through again to find servers with no queue. If none is found after this two iteration,
         * the customer will LEAVE.
         *
         * @return Next event after ARRIVE. Either WaitEvent, ServedEvent, LeaveEvent.
         */
        Event newEvent = new EventStub(new Customer(0, 0), 0);
        boolean eventSettled = false;
        for (int i = 0; i < shop.getServerList().size(); i++) {
            if (shop.getServerList().get(i).isAvailable() &&
                    (this.getTime() >= shop.getServerList().get(i).getNextAvailableTime())) {
                newEvent = new ServeEvent(this.getCustomer(), this.getTime(),
                        i, shop.getServerList().get(i));
                eventSettled = true;
                break;
            } else {
                continue;
            }
        }

        if (!eventSettled) {
            for (int i = 0; i < shop.getServerList().size(); i++) {
                if (shop.getServerList().get(i).canQueue()) {
                    newEvent = new WaitEvent(this.getCustomer(), this.getTime(),
                            i, shop.getServerList().get(i));
                    eventSettled = true;
                    break;
                } else {
                    continue;
                }
            }
        }

        if (!eventSettled) {
            newEvent = new LeaveEvent(this.getCustomer(), this.getTime());
        }
        return Pair.of(Optional.<Event>of(newEvent), shop);
    }

    double getWaitingTime() {
        return this.time - this.customer.getArrivalTime();
    }

    @Override
    public String toString() {
        return String.format("%.3f %s arrives", time, customer.getId());
    }
}



